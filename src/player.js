...
export class Player {
  ...
  teleport(point) { this.pos.set(point[0],point[1]+.02,point[2]);this.velocity.set(0,0,0);this.ledge=null;this.actionTime=0;this.state='idle';this.body.setNextKinematicTranslation({x:this.pos.x,y:this.pos.y+this.height/2,z:this.pos.z}); }
  ...
  #topClear(obj, dir) {
    const x=this.pos.x+dir.x*(.75+this.radius), z=this.pos.z+dir.z*(.75+this.radius);
  ...
  #parkour(input,dir) {
    ...
    if(!o.ledge || rise<.3 || rise>2.5 || !this.#topClear(o,dir) || !this.#changeCrouch(false)) return false;
    const depth=Math.abs(dir.x)*o.w+Math.abs(dir.z)*o.d;
    const to=v(this.pos.x+dir.x*(hit.distance+depth+.42),o.top+.03,this.pos.z+dir.z*(hit.distance+depth+.42));
    if(rise<=.9) {this.#startAction('vault',to,.64);return true;}
    if(rise<=1.35 || input.jump) {this.#startAction('pull',to,.92);return true;}
    this.ledge={obstacle:o,dir:dir.clone(),x:hit.point.x,z:hit.point.z,entering:true}; this.state='hang';this.pos.y=o.top-1.45;this.velocity.set(0,0,0);this.sound.play('climb');return true;
  }
  #tryDescend(input,dir){
    ...
    this.ledge={obstacle:o,dir:alongX?v(face,0,0):v(0,0,face),entering:true};this.state='hang';this.velocity.set(0,0,0);this.sound.play('climb');return true;
  }
  #ledgeStep(dt,input) {
    ...
    if(Math.abs(candidate.x-o.x)<o.w/2+.25 && Math.abs(candidate.z-o.z)<o.d/2+.25) this.pos.copy(candidate);
    if(!input.forward)this.ledge.entering=false;
    if((input.forward&&!this.ledge.entering) || input.jumpPressed) {
      const to=v(this.pos.x+dir.x*(o.d/2+.8),o.top+.03,this.pos.z+dir.z*(o.d/2+.8));
      if(this.#topClear(o,dir)){this.ledge=null;this.#startAction('pull',to,.98);} }
    this.#commit();
  }
  ...
  step(dt,input,cameraYaw) {
    ...
    if(this.jumpBuffer>0 && this.coyote>0 && this.#changeCrouch(false)){this.velocity.y=input.shift?7.1:6.6;this.grounded=false;this.coyote=0;this.jumpBuffer=0;this.state='jump';this.fallStart=this.pos.y;this.sound.play('jump');}
    ...
    if(!this.grounded) this.state=this.velocity.y>1?'jump':'airborne';
    else if(this.crouch)this.state=horizontal>.25?'crouchWalk':'crouch';
    else this.state=horizontal<.2?'idle':horizontal<3.3?'walk':'run';
    this.animations.play(this.state,this.state==='walk'?clamp(horizontal/2,.65,1.4):this.state==='run'?clamp(horizontal/5,.7,1.25):1);
