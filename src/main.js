...
app.innerHTML=`<div id="hud"><b>ParkourGame · прототип</b><div id="status">Загрузка…</div><small>WASD — движение · Shift — бег и паркур · Space — прыжок<br>Ctrl — скрытность · C — спуск · E — отпустить уступ<br>R — вернуть персонажа · Esc — меню · F3 — диагностика</small></div><div id="hint">Нажмите «Играть», чтобы захватить курсор</div><div id="diagnostics"></div><div id="animation-tools"><label>Проверка клипа <select id="clip-select"></select></label> <button id="clip-play">▶</button></div><div id="menu"><div class="panel"><h1>ParkourGame</h1><div class="muted">Тестовый уровень для проверки движения и анимаций</div><div class="row"><button id="resume">Играть</button><button id="reset">Вернуть персонажа</button></div><h2>Испытательная зона</h2><div class="row" id="zones"></div><h2>Настройки</h2><div id="settings"></div><h2>Клавиши</h2><div id="bindings"></div><h2>Ресурсы и ограничения</h2><div id="asset-info" class="muted">Загрузка модели…</div></div></div>`;
...
try {const report=await animations.load();player=new Player(world,level.obstacles,animations,settings,sound);window.__parkour={player,animations,report};ready=true;
  ...
  $('#asset-info').innerHTML=`Модель: Y Bot.fbx · высота FBX ${report.height.toFixed(2)} · масштаб ${report.scale.toFixed(4)}<br>Загружено клипов: ${Object.keys(report.clips).length}. ${report.missing.length?`Ошибки: <span class="error">${report.missing.join('; ')}</span>`:'Все выбранные клипы загружены.'}<br>Висение и боковое перемещение по уступу пока используют позу лазания. Свободного лазания по произвольной стене нет.`;
