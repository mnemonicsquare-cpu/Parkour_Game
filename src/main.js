...
const defaults={runSpeed:5,groundAcceleration:14,groundDeceleration:18,turnRate:11,climbMinHeightRatio:1,climbMaxRiseRatio:1.5,grabDistance:.78,grabWallGap:.06,landingInset:.18,climbSpeed:1.4,climbMinDuration:.32,climbMaxDuration:1.6,pullDuration:2.05,slideSpeed:6.5,slideDuration:1.05,slideDeceleration:3,slideHeight:.85,slideCooldown:.45,heavyHeight:4.5,sensitivity:.0026,cameraDistance:4.5,followSpeed:9,volume:.45,movementVolume:.7,ambientVolume:.35,fade:.18};
const settings={...defaults,...JSON.parse(localStorage.getItem('parkour-settings')||'{}')};
const savedBindings=JSON.parse(localStorage.getItem('parkour-bindings')||'{}');
const bindings={forward:'KeyW',back:'KeyS',left:'KeyA',right:'KeyD',slide:'ShiftLeft',grab:'KeyF',jump:'Space',descend:'KeyC',release:'KeyE',reset:'KeyR'};
for(const key of Object.keys(bindings))if(savedBindings[key]&&!['ShiftLeft','ShiftRight','ControlLeft','ControlRight'].includes(savedBindings[key]))bindings[key]=savedBindings[key];
const app=document.querySelector('#app');
app.innerHTML=`<div id="hud"><b>ParkourGame · прототип</b><div id="status">Загрузка…</div><small>WASD — бег · Shift — подкат · F — захват края<br>Space — прыжок, повторно в воздухе — кувырок<br>W/Space в висе — подтянуться · S+Space — оттолкнуться<br>E — отпустить край · C — спуск с крыши<br>R — возврат · Esc — меню · F3 — диагностика</small></div><div id="hint">Нажмите «Играть», чтобы захватить курсор</div><div id="diagnostics"></div><div id="animation-tools"><label>Проверка клипа <select id="clip-select"></select></label> <button id="clip-play">▶</button></div><div id="menu"><div class="panel"><h1>ParkourGame</h1><div class="muted">Тестовый уровень для проверки движения и анимаций</div><div class="row"><button id="resume">Играть</button><button id="reset">Вернуть персонажа</button></div><h2>Испытательная зона</h2><div class="row" id="zones"></div><h2>Настройки</h2><div id="settings"></div><h2>Клавиши</h2><div id="bindings"></div><h2>Ресурсы и ограничения</h2><div id="asset-info" class="muted">Загрузка модели…</div></div></div>`;
...
function fillSettings(){const labels={runSpeed:'Скорость бега',groundAcceleration:'Разгон',groundDeceleration:'Торможение',turnRate:'Скорость разворота',climbMinHeightRatio:'Мин. высота лазания × рост',climbMaxRiseRatio:'Досягаемость края × рост',grabDistance:'Дистанция захвата',grabWallGap:'Отступ от стены',landingInset:'Глубина выхода на крышу',climbSpeed:'Скорость подъёма',climbMinDuration:'Мин. время подъёма',climbMaxDuration:'Макс. время подъёма',pullDuration:'Время подтягивания',slideSpeed:'Начальная скорость подката',slideDuration:'Длительность подката',slideDeceleration:'Замедление подката',slideHeight:'Высота подката',slideCooldown:'Пауза между подкатами',heavyHeight:'Тяжёлое падение, м',sensitivity:'Чувствительность мыши',cameraDistance:'Расстояние камеры',followSpeed:'Скорость камеры',volume:'Общая громкость',movementVolume:'Громкость движений',ambientVolume:'Громкость окружения',fade:'Смешивание анимаций'};
  const ranges={runSpeed:[2,9,.1],groundAcceleration:[5,30,.5],groundDeceleration:[5,35,.5],turnRate:[3,20,.5],climbMinHeightRatio:[.8,1.3,.05],climbMaxRiseRatio:[1.1,2,.05],grabDistance:[.35,1.2,.05],grabWallGap:[.02,.2,.01],landingInset:[.1,.4,.01],climbSpeed:[.7,3,.1],climbMinDuration:[.2,.8,.02],climbMaxDuration:[1,2.5,.05],pullDuration:[1,4,.05],slideSpeed:[4,10,.1],slideDuration:[.5,2,.05],slideDeceleration:[1,8,.1],slideHeight:[.65,1.1,.05],slideCooldown:[0,1.5,.05],heavyHeight:[2,8,.1],sensitivity:[.0005,.008,.0001],cameraDistance:[2,8,.1],followSpeed:[2,20,.5],volume:[0,1,.05],movementVolume:[0,1,.05],ambientVolume:[0,1,.05],fade:[.03,.5,.01]};
  ...
  const bindLabels={forward:'Вперёд',back:'Назад',left:'Влево',right:'Вправо',slide:'Подкат',grab:'Захват края',jump:'Прыжок',descend:'Спуск',release:'Отпустить уступ',reset:'Возврат'};
...
fillSettings();
window.addEventListener('keydown',e=>{const waiting=document.querySelector('[data-waiting]');if(waiting){if(e.code==='ControlLeft'||e.code==='ControlRight'){e.preventDefault();return;}bindings[waiting.dataset.bind]=e.code;waiting.textContent=e.code;delete waiting.dataset.waiting;localStorage.setItem('parkour-bindings',JSON.stringify(bindings));e.preventDefault();return;}
...
function readInput(){return {forward:keys.has(bindings.forward),back:keys.has(bindings.back),left:keys.has(bindings.left),right:keys.has(bindings.right),grab:keys.has(bindings.grab),slidePressed:pressed.has(bindings.slide)||pressed.has('ShiftRight'),jump:keys.has(bindings.jump),jumpPressed:pressed.has(bindings.jump),descend:keys.has(bindings.descend),release:pressed.has(bindings.release)};}
...
try {const report=await animations.load();player=new Player(world,level.obstacles,animations,settings,sound);window.__parkour={player,animations,report};ready=true;
  ...
  $('#asset-info').innerHTML=`Модель: Y Bot.fbx · высота FBX ${report.height.toFixed(2)} · масштаб ${report.scale.toFixed(4)}<br>Загружено клипов: ${Object.keys(report.clips).length}. ${report.missing.length?`Ошибки: <span class="error">${report.missing.join('; ')}</span>`:'Все выбранные клипы загружены.'}<br>Новый Climbing завершает подтягивание на крышу. Вис, прыжок с разбега, спуск и повороты используют дополнительные клипы. Кувырок второго прыжка остаётся процедурным.`;
...
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.05,(now-previous)/1000);previous=now;frames++;fpsClock+=dt;if(fpsClock>.5){fps=Math.round(frames/fpsClock);fpsClock=0;frames=0;}
  if(ready&&!paused){accumulator=Math.min(.2,accumulator+dt);while(accumulator>=step){const state=readInput();if(pressed.has(bindings.reset))player.reset();player.step(step,state,yaw);world.step();pressed.clear();accumulator-=step;}
    ...
    if(debug){capsuleDebug.position.copy(player.pos).add(new THREE.Vector3(0,player.colliderHeight/2,0));capsuleDebug.scale.y=player.colliderHeight/player.height;
